...
fn the_reactor_must_return_a_success(world: &mut BasicsWorld, ) {
    assert!(world.reactor.find_result.is_some(), "Expected find_result to be Some, but it was None");
}

///
/// 
#[when(expr = "the reactor find function is called with the previously given attribute name")]
fn the_reactor_find_function_is_called_with_the_previously_given_attribute_name(world: &mut BasicsWorld, ) {
...
fn the_reactor_must_return_a_null_value(world: &mut BasicsWorld, ) {
    assert!(world.reactor.find_result.is_none(), "Expected find_result to be None, but it was Some");
}
