use super::{AsyncCallbackEntry, CallbackId};
...
/// Asynchronous generic attribute implementation that can work with any buffer type that implements GenericBuffer
#[derive(Clone, Debug)]
pub struct AsyncGenericAttribute<B: GenericBuffer> {
    /// Global Session
    session: Session,
    ...
    async_callbacks: Arc<Mutex<HashMap<CallbackId, AsyncCallbackEntry<B>>>>,
...
impl<B: GenericBuffer> AsyncGenericAttribute<B> {
    /// Create a new instance
    pub async fn new(session: Session, metadata: AttributeMetadata) -> Self {
        // Initialize async callbacks storage
        let async_callbacks = Arc::new(Mutex::new(
            HashMap::<CallbackId, AsyncCallbackEntry<B>>::new(),
        ));
        ...
        tokio::spawn({
            let async_callbacks = async_callbacks.clone();
            ...
            async move {
                while let Ok(sample) = subscriber.recv_async().await {
                    ...
                    let callbacks_map = async_callbacks.lock().await;
        ...
        F: Fn(&B) -> bool + Send + Sync + 'static,
    {
        ...
        // Remove the callback
        self.remove_async_callback(callback_id).await;
    ...
    pub async fn add_async_callback<F, C>(&self, callback: F, condition: Option<C>) -> CallbackId
    where
        ...
        C: Fn(&B) -> bool + Send + Sync + 'static,
    {
        ...
        *next_id += 1;
        ...
        let callback_entry = AsyncCallbackEntry {
    ...
    pub async fn remove_async_callback(&self, callback_id: CallbackId) -> bool {
        let mut callbacks = self.async_callbacks.lock().await;
        callbacks.remove(&callback_id).is_some()
    }

    /// Clear all async callbacks
    pub async fn clear_async_callbacks(&self) {
        let mut callbacks = self.async_callbacks.lock().await;
        callbacks.clear();
    }

    /// Get the number of registered async callbacks
    pub async fn async_callback_count(&self) -> usize {
        let callbacks = self.async_callbacks.lock().await;
        callbacks.len()
    }
...
pub type AsyncBooleanGenericAttribute = AsyncGenericAttribute<crate::fbs::BooleanBuffer>;
...
mod tests {
    ...
    async fn test_async_callback_registration() {
        ...
        let attribute: AsyncGenericAttribute<BooleanBuffer> =
        ...
        assert_eq!(attribute.async_callback_count().await, 1);

        // Test removing callback
        let removed = attribute.remove_async_callback(callback_id).await;
        assert!(removed);
        assert_eq!(attribute.async_callback_count().await, 0);
    }
    #[tokio::test]
    async fn test_async_callback_with_condition() {
        let config = Config::default();
        let session = zenoh::open(config).await.unwrap();
